...
public final class CommandChestSpawner implements TabExecutor {
      ...
      if (args.length > 4) {
         ...
         if (args.length == 1) {
            if (arg1.equals("closeInventory") && sender instanceof Player) {
               ...
               if (!sender.hasPermission("zpawner.settings")) {
                  sender.sendMessage(this.spawnerAPI.getLanguage().errorPermission);
                  return true;
            ...
            if (args.length == 2) {
               if (arg1.equals("sell") && sender instanceof Player) {
                  ...
                  return true;
               } else if (!sender.hasPermission("zpawner.distribute")) {
                  sender.sendMessage(this.spawnerAPI.getLanguage().errorPermission);
                  return true;
               ...
               if (args.length != 3) {
                  ...
                  if (!arg1.equalsIgnoreCase(this.spawnerAPI.getLanguage().commandItemSpawnerGive)) {
                     ...
                     return true;
                  } else if (!sender.hasPermission("zpawner.give")) {
                     sender.sendMessage(this.spawnerAPI.getLanguage().errorPermission);
                     return true;
   ...
   public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
      if (!sender.hasPermission("zpawner.give") && !sender.hasPermission("zpawner.distribute") && !sender.hasPermission("zpawner.settings")) {
